...
| `src/pdex2/_math.py`    | Numba JIT-compiled `fold_change()`, `percent_change()`, and `mwu()` wrappers; `pseudobulk()` dispatcher |
| `src/pdex2/_utils.py`   | `set_numba_threadpool()` — sets Numba thread count before JIT warmup; `_detect_is_log1p()` heuristic    |
...
```python
from pdex2 import pdex, DEFAULT_REFERENCE
```

## Dependencies

Managed with `uv`. Key packages: `anndata`, `numpy`, `polars`, `pyarrow` (for `as_pandas`), `scipy`, `tqdm`, `numba-mwu` (from GitHub via SSH). Dev tools: `pytest`, `ruff`, `ty`.
